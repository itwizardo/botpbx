...
// Map Asterisk channel state to frontend state
function mapAsteriskState(state: string): 'ringing' | 'up' | 'busy' | 'noanswer' {
...
export function registerCallRoutes(server: FastifyInstance, ctx: ApiContext): void {
  ...
  server.get('/active', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      // Map to frontend ActiveCall interface format
      const calls = channels.map(ch => ({
        uniqueId: ch.channel,
        channel: ch.channel,
        callerId: ch.callerIdNum || 'Unknown',
        callerName: ch.callerIdName || '',
        destination: ch.extension || '',
        context: ch.context,
        state: mapAsteriskState(ch.state),
        startTime: now - (ch.duration || 0),
        answerTime: ch.state?.toLowerCase() === 'up' ? now - (ch.duration || 0) : null,
