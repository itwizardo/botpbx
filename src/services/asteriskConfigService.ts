...
exten => _X.,1,NoOp(Inbound call to \${EXTEN} from \${CALLERID(num)})
 ...
 same => n,Set(RECORDING_FILE=/var/spool/asterisk/monitor/\${STRFTIME(\${EPOCH},,%Y%m%d-%H%M%S)}-\${UNIQUEID}.wav)
...
exten => s,1,NoOp(Test IVR call initiated)
 ...
 same => n,Wait(1)
 same => n,Set(CALL_START_TIME=\${EPOCH})
 same => n,Set(RECORDING_FILE=/var/spool/asterisk/monitor/test-ivr-\${STRFTIME(\${EPOCH},,%Y%m%d-%H%M%S)}-\${UNIQUEID}.wav)
 same => n,MixMonitor(\${RECORDING_FILE})
 same => n,AGI(agi://\${AGI_HOST}:\${AGI_PORT}/ivr)
 same => n,Hangup()
...
exten => s,1,NoOp(Outbound dialer call)
 ...
 same => n,Wait(0.5)
 same => n,Set(CALL_START_TIME=\${EPOCH})
 same => n,Set(RECORDING_FILE=/var/spool/asterisk/monitor/campaign-\${STRFTIME(\${EPOCH},,%Y%m%d-%H%M%S)}-\${UNIQUEID}.wav)
 same => n,MixMonitor(\${RECORDING_FILE})
...
exten => s,1,NoOp(Test call via trunk)
 ...
 same => n,Wait(1)
 same => n,Set(CALL_START_TIME=\${EPOCH})
 same => n,Set(RECORDING_FILE=/var/spool/asterisk/monitor/test-\${STRFTIME(\${EPOCH},,%Y%m%d-%H%M%S)}-\${UNIQUEID}.wav)
 same => n,MixMonitor(\${RECORDING_FILE})
...
exten => h,1,NoOp(Test call ended - cause: \${HANGUPCAUSE})
 same => n,Set(CALL_DURATION=$[\${EPOCH}-\${CALL_START_TIME}])
 same => n,AGI(agi://127.0.0.1:4573/test-call-end)
...
exten => s,1,NoOp(IVR Test call - menu ID: \${IVR_MENU_ID})
 ...
 same => n,Wait(1)
 same => n,Set(CALL_START_TIME=\${EPOCH})
 same => n,Set(RECORDING_FILE=/var/spool/asterisk/monitor/ivr-test-\${STRFTIME(\${EPOCH},,%Y%m%d-%H%M%S)}-\${UNIQUEID}.wav)
 same => n,MixMonitor(\${RECORDING_FILE})
 same => n,AGI(agi://\${AGI_HOST}:\${AGI_PORT}/ivr)
 same => n,Hangup()
...
exten => s,1,NoOp(AI Agent test call - Agent: \${AGENT_ID}, UUID: \${CALL_UUID})
 ...
 same => n,Wait(1)
 same => n,Set(CALL_START_TIME=\${EPOCH})
 same => n,Set(RECORDING_FILE=/var/spool/asterisk/monitor/ai-\${STRFTIME(\${EPOCH},,%Y%m%d-%H%M%S)}-\${UNIQUEID}.wav)
 same => n,MixMonitor(\${RECORDING_FILE})
...
exten => h,1,NoOp(AI Agent call ended - cause: \${HANGUPCAUSE})
 same => n,Set(CALL_DURATION=$[\${EPOCH}-\${CALL_START_TIME}])
 same => n,AGI(agi://127.0.0.1:4573/ai-call-end)
`;
  }
