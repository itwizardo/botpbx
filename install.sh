...
# If a dedicated botpbx Linux user exists, add to asterisk group for config write access
if id -u botpbx &>/dev/null; then
    usermod -a -G asterisk botpbx
    log "  ✓ Added botpbx user to asterisk group"
fi
...
# Always create proper pjsip.conf (backup existing if present)
if [ -f /etc/asterisk/pjsip.conf ]; then
    cp /etc/asterisk/pjsip.conf /etc/asterisk/pjsip.conf.samples.bak
    log "  Backed up existing pjsip.conf to pjsip.conf.samples.bak"
fi

cat > /etc/asterisk/pjsip.conf << PJSIPEOF
; ===============================================
; BotPBX PJSIP Configuration
; Auto-generated by BotPBX installer
; ===============================================
...
; TLS transport for providers like Twilio
[transport-tls-twilio]
type=transport
protocol=tls
bind=0.0.0.0:5061
method=tlsv1_2
...
; WebRTC template for browser phone
[webrtc-template](!)
type=endpoint
...
; ===============================================
; INCLUDES - Auto-generated configs
; ===============================================
...
[globals]
; Global variables - set TRUNK_NAME to your default outbound trunk
TRUNK_NAME=trunk-provider
AGI_HOST=127.0.0.1
AGI_PORT=4573
...
; International numbers with + prefix (e.g., +15551234567)
exten => _+X.,1,NoOp(International call to ${EXTEN})
 same => n,Dial(PJSIP/${EXTEN}@${TRUNK_NAME},120,tT)
 same => n,Hangup()

; Numbers starting with 00 (international format)
exten => _00X.,1,NoOp(International call to ${EXTEN})
 same => n,Dial(PJSIP/+${EXTEN:2}@${TRUNK_NAME},120,tT)
 same => n,Hangup()

; Dial external numbers (outbound) - US format
exten => _NXXNXXXXXX,1,NoOp(Outbound call to ${EXTEN})
 same => n,Dial(PJSIP/+1${EXTEN}@${TRUNK_NAME},60,tT)
 same => n,Hangup()

; E.164 format outbound (11 digits with 1 prefix)
exten => _1NXXNXXXXXX,1,NoOp(Outbound call to ${EXTEN})
 same => n,Dial(PJSIP/+${EXTEN}@${TRUNK_NAME},60,tT)
 same => n,Hangup()
...
# HTTP/HTTPS for Nginx reverse proxy and Let's Encrypt
