...
const getServerUrl = () => {
  ...
  // If on HTTPS (via nginx), use nginx proxy path
  // Otherwise connect directly to Asterisk WebSocket port
  if (protocol === 'wss' && (!port || port === '443')) {
    // Behind nginx - use proxied path
    return `${protocol}://${host}/asterisk-ws`;
  } else {
    // Direct access - connect to Asterisk port directly
    const wsPort = protocol === 'wss' ? 8089 : 8088;
...
export function useWebRTCPhone(): UseWebRTCPhoneReturn {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      // Use same URL logic as api.ts - empty string when behind nginx proxy
