...
export function createWebSocketClient(token: string): WebSocketClient {
  ...
  // Only use env var if it's set and not localhost (matches api.ts behavior)
  ...
  if (envWsUrl && !envWsUrl.includes('localhost')) {
    ...
    // Derive from current location - works from any computer
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const host = window.location.hostname;
    const currentPort = window.location.port;

    // If accessing via standard ports (80/443) or no port, use nginx proxy
    // Otherwise connect directly to API port
    if (!currentPort || currentPort === '80' || currentPort === '443') {
      // Behind nginx reverse proxy - use same host/port
      wsUrl = `${protocol}//${host}`;
    } else {
      // Direct access - connect to API port
      wsUrl = `${protocol}//${host}:3000`;
    }
  } else {
    // Server-side fallback
    wsUrl = envWsUrl || 'ws://localhost:3000';
  }
