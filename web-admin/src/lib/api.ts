...
// When behind nginx (HTTPS on 443), use relative paths through nginx proxy
function detectApiBaseUrl(): string {
  ...
  if (typeof window !== 'undefined') {
    ...
    // If accessing via standard HTTPS (443) or HTTP (80) through nginx, use relative URL
    // nginx proxies /api/ to the backend
    if (protocol === 'https:' || (protocol === 'http:' && (!port || port === '80'))) {
      return ''; // Use relative URLs - nginx handles the proxy
    }
    // Direct access (e.g., localhost:3001), use port 3000
