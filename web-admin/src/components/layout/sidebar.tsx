...
function NavLink({
  ...
  const linkContent = (
    <Link
      ...
      prefetch={true}
      className={cn(
        'flex items-center gap-2 rounded-md px-2.5 py-1.5 text-sm font-medium transition-all duration-150',
...
export function Sidebar() {
  ...
  return (
    <TooltipProvider>
      <aside
        className={cn(
          'fixed left-0 top-0 z-40 h-screen border-r border-white/5 bg-sidebar/95 backdrop-blur-xl transition-all duration-300 hidden md:block',
      ...
      >
        <div className="flex h-full flex-col">
          {/* Logo */}
          <div className={cn(
            "flex items-center justify-center border-b transition-all duration-300",
            sidebarCollapsed ? "h-16 p-2" : "h-32"
          )}>
            <Link href="/">
              <Image
                src="/login.gif"
          ...
          <div className="flex-1 overflow-y-auto px-2 py-2">
            ...
            {/* Bottom Navigation - inside same container, no gap */}
            <nav className="flex flex-col gap-0.5 mt-2 pt-2 border-t">
